use dxkb_common::{dev_info, dev_warn, KeyState};
...
pub fn standard_key_handle<S, Kb: SplitKeyboardLike<S>>(kb: &mut Kb, key: KeyboardUsage, key_state: KeyState) {
    do_on_state!(key_state, { kb.hid_report_mut().add_key(key) }, { kb.hid_report_mut().rm_key(key) });
}

pub fn function_key_handle<S: KeyboardStateLike, Kb: SplitKeyboardLike<S>>(kb: &mut Kb, key: &BuiltinFunctionKey, key_state: KeyState) {
    match key {
        BuiltinFunctionKey::PushNextLayer => {
            do_on_state!(key_state, {
...
impl HandleKey for DefaultKey {
    ...
    fn handle_key_state_change<S: KeyboardStateLike, Kb: SplitKeyboardLike<S>>(&self, kb: &mut Kb, _user: &mut Self::User, key_state: KeyState) {
        match self {
            DefaultKey::NoOp => {},

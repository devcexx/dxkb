...
/// Represents a type that arranges the data stored in the Key matrix
/// one way or another, depending on the type that conforms to this
/// trait. Given that it knows exactly how this data is arranged, it
/// also provides methods to read or write this data.
...
/// A [`MatrixLayout`] that defines an arrangement of the matrix data
/// in rows, in which each button has a size of 1 bit. The internal
/// representation will be defined as a primitive numeric value (u8,
/// u16, u32, etc), that will depend on the value of `ROWS`.
pub struct RowLayout<const ROWS: u8> {}

impl MatrixLayout for RowLayout<4> {
    type RowDataType = u8;

    fn set_state(elem: &mut Self::RowDataType, col: u8, value: KeyState) {
        match value {
            // TODO I guess that bit banding doesn't really worth at this point?

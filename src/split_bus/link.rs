...
use crate::periph::{BusPollError, BusRead, BusTransferError, BusWrite};
...
pub struct SplitBus<Msg, B: BusWrite + BusRead, const TX_QUEUE_LEN: usize> {
    bus: B,
...
impl<Msg: Debug + DeserializeOwned + Serialize, B: BusWrite + BusRead, const TX_QUEUE_LEN: usize> SplitBus<Msg, B, TX_QUEUE_LEN> where [(); MaxFrameLength::<Msg>::MAX_FRAME_LENGTH]: {
    ...
    fn do_rx<F: FnMut(&Msg) -> ()>(&mut self, mut recvf: F) {
        ...
        while {
            let should_continue = match self.bus.poll_next(&mut rxbuf) {
                Ok(frame_len) => {
                    let frame = Self::decode_frame(&rxbuf[0..frame_len as usize]).unwrap();

                    // TODO Is this correct? Should I update this field for any frame? (Probably yes).
                    self.last_recv_frame_ticks = DWT::cycle_count();
...
//                    recvf(&frame);
    ...
    fn transfer_frame(bus: &mut B, last_sent_frame_ticks: &mut u32, frame: &Frame<Msg>) -> Result<(), BusTransferError> {
        ...
        if matches!(res, Ok(_)) {
            ...
            *last_sent_frame_ticks = DWT::cycle_count();
        }
    ...
    fn do_tx(&mut self) {
        ...
        if self.link_state && !self.bus.is_tx_busy() {
            if let Some(frame) = self.control_tx_queue.peek() {
                let _ = Self::transfer_frame(&mut self.bus, &mut self.last_sent_frame_ticks, frame);
            }
        };
        ...
        if self.last_sent_frame_ticks > LINK_IDLE_PROBE_INTERVAL_MS && !self.bus.is_tx_busy() {
            ...
            let _ = Self::transfer_frame(&mut self.bus, &mut self.last_sent_frame_ticks, &Frame::Probe);
        }
    }

    // TODO Do not expose the frame. This is exclusive for the L2 protocol. Only the final messages should be exposed.
    pub fn poll<F: FnMut(&Frame<Msg>) -> ()>(&mut self, recvf: F) {
        self.do_rx(recvf);

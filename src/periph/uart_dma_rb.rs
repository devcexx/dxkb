...
use crate::{dev_info, dev_warn, free_with_muts};

use super::{BusPollError, BusRead, BusWrite};
...
impl<const BUF_LEN: usize, const MAX_FRAME_COUNT: usize> DmaRingBufferWriteSide<BUF_LEN, MAX_FRAME_COUNT> {
    ...
    fn push_rb_frame<F: FnOnce(u16) -> RbSection>(&mut self, ndt: u16, ctor: F) -> Result<(), CloseCurrentFrameError> {
    ...
    fn close_current_frame(&mut self, ndt: u16) -> Result<(), CloseCurrentFrameError> {
        self.push_rb_frame(ndt, |len| RbSection { len, kind: RbSectionKind::Frame })
    }

    #[inline(always)]
    pub fn discard_current_frame(&mut self, ndt: u16) -> Result<(), CloseCurrentFrameError> {
        self.push_rb_frame(ndt, |len| RbSection { len, kind: RbSectionKind::Discarded })
    }
...
Usart: DMASet<RxStream, RX_CH, PeripheralToMemory>,
 {
     pub const fn assert_max_rx_buf_size_valid(value: usize) {
         assert!(value <= 65535, "Rx buf size cannot be bigger than 65535 bytes");
     }
     ...
     pub fn init(
         ...
         const {
             Self::assert_max_rx_buf_size_valid(MAX_RX_BUF)
         };
     ...
     fn init_rx_dma_xfer(&mut self, peri_addr: u32) {
         unsafe {
             self.rx_stream.disable()
         };
         self.rx_stream.set_number_of_transfers(MAX_RX_BUF as u16);
         self.rx_stream.set_channel(<ChannelX::<RX_CH> as Channel>::VALUE);
         self.rx_stream.set_direction(PeripheralToMemory::direction());
         self.rx_stream.set_memory_address(self.rx_buf.buf.as_ptr() as u32);
         ...
         self.rx_stream.set_fifo_threshold(FifoThreshold::QuarterFull);
...
Usart: DMASet<RxStream, RX_CH, PeripheralToMemory>,
 {
    fn transfer(&mut self, buf: &[u8]) -> Result<(), super::BusTransferError> {
        todo!()
    }
...
Usart: DMASet<RxStream, RX_CH, PeripheralToMemory>,
 {
    fn poll_next(&self, buf: &mut [u8]) -> Result<u16, super::BusPollError> {
